...
PetscErrorCode Contra2Cart(UserCtx *user)
{
  ...
  for (k=lzs; k<lze; k++) {
	...
	if ( nvert[k][j][i] < 0.1 || nvert[k][j][i] > innerblank) {
	  mat[0][0] = 0.5 * (csi[k][j][i-1].x + csi[k][j][i].x); // Each csi is associated with a face of the control cell, hence, averaging is done to estimate the csi at the central face of the control cell.
