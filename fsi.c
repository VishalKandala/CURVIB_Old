#include "variables.h"
extern PetscInt radi,fish,pizza,TwoD,cop, wing, ti, rheology,sediment,tistart;
...
/*  if (!pizza && !fish && !cop && !wing && !rheology && !sediment) {
  if (fabs(A_xSum)>1e-6)
    F_xSum=F_xSum/A_xSum*2.;
  if (fabs(A_ySum)>1e-6)
    F_ySum=F_ySum/A_ySum*2.;
...
// projection
PetscInt IB_node_total_each_cpu(UserCtx *user, PetscInt ibi)
{
...
//-------------------------------------------------------------------------
PetscErrorCode Initialize_Projecting(IBMNodes *ibm ) {
  
  PetscInt  n_elmt=ibm->n_elmt;
  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->tau0_p));
  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->tauN_p));
  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->pres_p));  

  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->Bvel_u_p));  
  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->Bvel_v_p));  
  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->Bvel_w_p));  

  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->tau0_n));
  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->tauN_n));
  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->pres_n));  

  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->Bvel_u_n));  
  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->Bvel_v_n));  
  PetscMalloc(n_elmt*sizeof(PetscReal), &(ibm->Bvel_w_n)); 
 
  return(0);
}

//------------------------------------------------------------------------
PetscErrorCode  Projecting(UserCtx *user, IBMNodes *ibm, PetscInt ibi) {
  ...
  PetscReal     *NIB_p, *NIB_n;
  PetscInt      processors, rank;
  PetscInt      number_of_IBnodes=0, total_IBnodes=0;
  ...
  PetscInt      *current_sidep, *sidep_buffer;
  PetscInt      i, j, k, elmt;
  ...
  number_of_IBnodes = IB_node_total_each_cpu(user, ibi);
  
  PetscBarrier(PETSC_NULL);
  // Finding total number of IB nodes in all cpu's to all cpu's knowing the total
  MPI_Allreduce(&number_of_IBnodes, &total_IBnodes, 1, MPIU_INT, MPI_SUM, MPI_COMM_WORLD);
  PetscPrintf(PETSC_COMM_WORLD, "Projecting body %d totol IB nodes %d\n", ibi, total_IBnodes);
  // All cpu's sending their number of IB nodes to the root cpu naming "stride" in root
  MPI_Gather(&number_of_IBnodes, 1, MPIU_INT, stride, 1, MPIU_INT, 0, MPI_COMM_WORLD);
  PetscBarrier(PETSC_NULL);
  // Now root Bcasts number of each cpu's IB node as stride[i] to all other processors
  MPI_Bcast(stride, processors, MPIU_INT, 0, MPI_COMM_WORLD);
  ...
  PetscMalloc(number_of_IBnodes*sizeof(PetscInt), &current_sidep);
  ...
  PetscMalloc(total_IBnodes*sizeof(PetscInt), &sidep_buffer);
  
  for(i=0; i<total_IBnodes; i++) {
    element_buffer[i] = -100;
    tau0_buffer[i] = 0.;
    tauN_buffer[i] = 0.;
    pres_buffer[i] = 0.;
    uvel_buffer[i] = 0.;
  ...
  // Now putting the IB info into a local current array to be sent to the root process later
  ...
  while (current) {
    ...
    double     nfx2=ibm->nf_x[elmt], nfy2=ibm->nf_y[elmt], nfz2=ibm->nf_z[elmt];	
    PetscReal  nfx, nfy, nfz, dr;		
    ...
    if (ii<number_of_IBnodes) {
      ...
      if (i>=lxs && i<lxe && j>=lys && j<lye && k>=lzs && k<lze) {
	if ((nfx*nfx2+nfy*nfy2+nfz*nfz2)>0) current_sidep[ii] = 1; 
	else current_sidep[ii] = 0;
	...
	if (wallfunction) 
	  ...
	  current_tau0[ii]= sqrt(Udef.x*Udef.x +
				 Udef.y*Udef.y +
				 Udef.z*Udef.z)/sb/user->ren;
	  
	  current_tauN[ii] = ((Ub.x-Ua.x)*nfx +
			      (Ub.y-Ua.y)*nfy +
			      (Ub.z-Ua.z)*nfz )/sb/user->ren*2.;
	}
  ...
  MPI_Gatherv(current_element, number_of_IBnodes, MPIU_INT, element_buffer, rec, dis, MPIU_INT, 0, MPI_COMM_WORLD); 
  MPI_Gatherv(current_sidep, number_of_IBnodes, MPIU_INT, sidep_buffer, rec, dis, MPIU_INT, 0, MPI_COMM_WORLD); 
  
  MPI_Gatherv(current_tau0, number_of_IBnodes, MPIU_REAL, tau0_buffer, rec, dis, MPIU_REAL, 0, MPI_COMM_WORLD);
  MPI_Gatherv(current_tauN, number_of_IBnodes, MPIU_REAL, tauN_buffer, rec, dis, MPIU_REAL, 0, MPI_COMM_WORLD);
  MPI_Gatherv(current_pres, number_of_IBnodes, MPIU_REAL, pres_buffer, rec, dis, MPIU_REAL, 0, MPI_COMM_WORLD);
  MPI_Gatherv(current_uvel, number_of_IBnodes, MPIU_REAL, uvel_buffer, rec, dis, MPIU_REAL, 0, MPI_COMM_WORLD);
  MPI_Gatherv(current_vvel, number_of_IBnodes, MPIU_REAL, vvel_buffer, rec, dis, MPIU_REAL, 0, MPI_COMM_WORLD);
  MPI_Gatherv(current_wvel, number_of_IBnodes, MPIU_REAL, wvel_buffer, rec, dis, MPIU_REAL, 0, MPI_COMM_WORLD);
  ...
  if(rank==0) {
    PetscMalloc(ibm->n_elmt*sizeof(PetscReal), &NIB_p);
    PetscMalloc(ibm->n_elmt*sizeof(PetscReal), &NIB_n);
    // clear former info
    for(i=0; i<ibm->n_elmt;i++){
      ibm->Bvel_u_p[i] = 0.0;
      ibm->Bvel_v_p[i] = 0.0;
	...
	if (sidep_buffer[i]==1) { // positive side
	  ibm->Bvel_u_p[elmt] += uu;
	  ibm->Bvel_v_p[elmt] += vv;
	  ibm->Bvel_w_p[elmt] += ww;
	  ibm->pres_p[elmt] += pres;
	  ibm->tau0_p[elmt] += tau0;
	  ibm->tauN_p[elmt] += tauN;
	  NIB_p[elmt] += 1.;
	} else if (sidep_buffer[i]==0) { // negative side
	  ibm->Bvel_u_n[elmt] += uu;
	  ibm->Bvel_v_n[elmt] += vv;
	  ibm->Bvel_w_n[elmt] += ww;
	  ibm->pres_n[elmt] += pres;
	  ibm->tau0_n[elmt] += tau0;
	  ibm->tauN_n[elmt] += tauN;
	  NIB_n[elmt] += 1.;
	}
      }
    } // End of all IB nodes 
    ...
    for(i=0; i<ibm->n_elmt; i++){
      if(NIB_p[i]>0.1) {
	ibm->Bvel_u_p[i] /= NIB_p[i];
	ibm->Bvel_v_p[i] /= NIB_p[i];
	ibm->Bvel_w_p[i] /= NIB_p[i];
	ibm->pres_p[i] /= NIB_p[i];
	ibm->tau0_p[i] /= NIB_p[i];
	ibm->tauN_p[i] /= NIB_p[i];
      }
      
      if(NIB_n[i]>0.1) {
	ibm->Bvel_u_n[i] /= NIB_n[i];
	ibm->Bvel_v_n[i] /= NIB_n[i];
	ibm->Bvel_w_n[i] /= NIB_n[i];
	ibm->pres_n[i] /= NIB_n[i];
	ibm->tau0_n[i] /= NIB_n[i];
	ibm->tauN_n[i] /= NIB_n[i];
      }
    } 
	...
	if(NIB_p[nelmt]<0.5)     {  
	  ...
	  for(nii=0; nii<ibm->n_elmt; nii++)    {
		...
		riter = riter + 1.;
		ibm->pres_p[ nelmt ] = ( ibm->pres_p[ nii ] + ibm->pres_p[ nelmt ] * ( riter - 1. ) ) / riter;
		ibm->tau0_p[ nelmt ] = ( ibm->tau0_p[ nii ] + ibm->tau0_p[ nelmt ] * ( riter - 1. ) ) / riter;
		ibm->tauN_p[ nelmt ] = ( ibm->tauN_p[ nii ] + ibm->tauN_p[ nelmt ] * ( riter - 1. ) ) / riter;
		ibm->Bvel_u_p[ nelmt] = ( ibm->Bvel_u_p[ nii ] + ibm->Bvel_u_p[ nelmt ] * ( riter - 1. ) ) / riter;
		ibm->Bvel_v_p[ nelmt] = ( ibm->Bvel_v_p[ nii ] + ibm->Bvel_v_p[ nelmt ] * ( riter - 1. ) ) / riter;
		ibm->Bvel_w_p[ nelmt] = ( ibm->Bvel_w_p[ nii ] + ibm->Bvel_w_p[ nelmt ] * ( riter - 1. ) ) / riter;
	      }
	    }
	  }
	}
	...
	if(NIB_n[nelmt]<0.5)     {  
	  ...
	  for(nii = 0; nii < ibm->n_elmt; nii++)    {
	    ...
	    if(nelmt!=nii  && ((ibm->nf_x[nelmt]*ibm->nf_x[nii] +
		 ...
		 ibm->nv3[ nelmt ] == ibm->nv3[ nii ])       {
		riter = riter + 1.;
		ibm->pres_n[ nelmt ] = ( ibm->pres_n[ nii ] + ibm->pres_n[ nelmt ] * ( riter - 1. ) ) / riter;
		ibm->tau0_n[ nelmt ] = ( ibm->tau0_n[ nii ] + ibm->tau0_n[ nelmt ] * ( riter - 1. ) ) / riter;
		ibm->tauN_n[ nelmt ] = ( ibm->tauN_n[ nii ] + ibm->tauN_n[ nelmt ] * ( riter - 1. ) ) / riter;
		ibm->Bvel_u_n[ nelmt] = ( ibm->Bvel_u_n[ nii ] + ibm->Bvel_u_n[ nelmt ] * ( riter - 1. ) ) / riter;
		ibm->Bvel_v_n[ nelmt] = ( ibm->Bvel_v_n[ nii ] + ibm->Bvel_v_n[ nelmt ] * ( riter - 1. ) ) / riter;
		ibm->Bvel_w_n[ nelmt] = ( ibm->Bvel_w_n[ nii ] + ibm->Bvel_w_n[ nelmt ] * ( riter - 1. ) ) / riter;
	      }
	    }
	  }
	}
    ...
    MPI_Bcast(ibm->Bvel_u_p, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
    MPI_Bcast(ibm->Bvel_v_p, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
    MPI_Bcast(ibm->Bvel_w_p, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
    MPI_Bcast(ibm->pres_p, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
    MPI_Bcast(ibm->tau0_p, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
    MPI_Bcast(ibm->tauN_p, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
    
    MPI_Bcast(ibm->Bvel_u_n, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
    MPI_Bcast(ibm->Bvel_v_n, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
    MPI_Bcast(ibm->Bvel_w_n, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
    MPI_Bcast(ibm->pres_n, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
    MPI_Bcast(ibm->tau0_n, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
    MPI_Bcast(ibm->tauN_n, ibm->n_elmt, MPIU_REAL, 0, MPI_COMM_WORLD);
...
PetscErrorCode Output_Projecting( IBMNodes * ibm )
{
  ...
  if (!rank) {
	...
	temp = ibm->pres_p[i] * specific_weight * Vel * Vel;
	if (Hg)
...
PetscErrorCode Projecting_VTKOut(IBMNodes *ibm, PetscInt ibi, PetscInt ti) {
  ...
  if (!rank) {
    FILE  *f;
    char  filen[80];
    sprintf(filen, "stress%2.2d_%5.5d.vtk", ibi, ti);
    ...
    for (i=0; i<ibm->n_elmt; i++) {
      PetscFPrintf(PETSC_COMM_WORLD, f, "%f %f %f\n", ibm->Bvel_u_p[i], ibm->Bvel_v_p[i], ibm->Bvel_w_p[i]);
    }
    PetscFPrintf(PETSC_COMM_WORLD, f,  "VECTORS u_proj- float\n");
    for (i=0; i<ibm->n_elmt; i++) {
      PetscFPrintf(PETSC_COMM_WORLD, f, "%f %f %f\n", ibm->Bvel_u_n[i], ibm->Bvel_v_n[i], ibm->Bvel_w_n[i]);
    }
    fclose(f); 
  }
