...
PetscErrorCode MG_Initial(UserMG *usermg, IBMNodes *ibm) {
  ...
  for (level=0; level<usermg->mglevels; level++) {
    ...
    for (bi=0; bi<block_number; bi++) {
      ...
      mgctx[level].user[bi].isc = isc[bi];
      mgctx[level].user[bi].jsc = jsc[bi];
      mgctx[level].user[bi].ksc = ksc[bi];
  ...
  PetscInt imm, kmm, jmm;
  if (generate_grid) {
    PetscOptionsGetInt(PETSC_NULL, "-im", &imm, PETSC_NULL);
    PetscOptionsGetInt(PETSC_NULL, "-jm", &jmm, PETSC_NULL);
    PetscOptionsGetInt(PETSC_NULL, "-km", &kmm, PETSC_NULL);
  }
  ...
  for (bi=0; bi<block_number; bi++) {
    ...
    DMLocalToGlobalBegin(user[bi].fda, Coor, INSERT_VALUES, gCoor); //Mohsen Dec 2015
  ...
  for (level=usermg->mglevels-1; level>=0; level--) {
	...
	if (phave)
	  {
	    VecDuplicate(user[bi].Csi, &user[bi].Ucat_sum0);
	    VecDuplicate(user[bi].Csi, &user[bi].Ucat_sum1);
	    VecDuplicate(user[bi].Csi, &user[bi].Ucat_sum2);	
	    VecDuplicate(user[bi].Csi, &user[bi].Ucat_sum3);
	    VecSet(user[bi].Ucat_sum0,0);
	    VecSet(user[bi].Ucat_sum1,0);
	    VecSet(user[bi].Ucat_sum2,0);
	    VecSet(user[bi].Ucat_sum3,0);
	    
	    VecDuplicate(user[bi].Aj, &user[bi].P_sum0);
	    VecDuplicate(user[bi].Aj, &user[bi].P_sum1);
	    VecDuplicate(user[bi].Aj, &user[bi].P_sum2);
	    VecDuplicate(user[bi].Aj, &user[bi].P_sum3);
	    VecSet(user[bi].P_sum0,0);
	    VecSet(user[bi].P_sum1,0);
	    VecSet(user[bi].P_sum2,0);
	    VecSet(user[bi].P_sum3,0);
	  }
	VecSet(user[bi].Ucat_cross_sum,0);
	VecSet(user[bi].Ucat_square_sum,0);
	
	VecDuplicate(user[bi].Aj, &user[bi].P_sum);
	VecDuplicate(user[bi].Aj, &user[bi].lUstar);
	VecDuplicate(user[bi].Aj, &user[bi].lUstar_sum);
	VecSet(user[bi].P_sum,0);
	VecSet(user[bi].lUstar_sum,0);
      }
	...
	VecDuplicate(user[bi].lCsi, &(user[bi].lUcont_rm1));
      ...
      VecDuplicate(user[bi].lAj, &(user[bi].lnvert_test));
...
PetscErrorCode MG_Finalize(UserMG *usermg)
{
  ...
  for (level=usermg->mglevels-1; level>=0; level--) {
	...
	VecDestroy(&user[bi].Ucat_sum);
	   ...
	   VecDestroy(&user[bi].Ucat_sum0);
	   VecDestroy(&user[bi].Ucat_sum1);
	   VecDestroy(&user[bi].Ucat_sum2);
	   VecDestroy(&user[bi].Ucat_sum3);
	...
	VecDestroy(&user[bi].Ucont_MG);
      ...
      VecDestroy(&user[bi].lnvert_test);
