...
PetscInt   NumberOfBodies=1, BHV=0, inv_flg=0;
PetscReal  L_dim=1.;
...
PetscErrorCode VTKOut(UserCtx *user) {
  ...
  array[Iloc] = 0.125 *
	   (nvert[k][j][i] + nvert[k][j][i+1] +
	    ...
	    if(nvert[k][j][i] == 1.4){
//	    PetscSynchronizedPrintf(PETSC_COMM_WORLD,"nvert,array - Iloc,k,j,i: %le,%le, %d,%d,%d,%d \n",nvert[k][j][i],array[Iloc],Iloc,k,j,i);
		}
	  }
	}
      }
