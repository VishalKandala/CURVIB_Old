...
{// updates metrics  at ghost nodes for periodci bounadry condition
  ...
  if (user->bctype[0]==7){
	...
	for (j=gys; j<gye; j++) {
	  i=0;
	  csi[k][j][i]= csi[k][j][i-2];
	  eta[k][j][i]= eta[k][j][i-2];
	  zet[k][j][i]= zet[k][j][i-2];
	  ...
	  csi[k][j][i-1]= csi[k][j][i-3];
	  eta[k][j][i-1]= eta[k][j][i-3];
	  zet[k][j][i-1]= zet[k][j][i-3];
  ...
  if (user->bctype[2]==7){
	...
	for (i=gxs; i<gxe; i++) {
	  j=0;
	  csi[k][j][i]= csi[k][j-2][i];
	  eta[k][j][i]= eta[k][j-2][i];
	  zet[k][j][i]= zet[k][j-2][i];
	  ...
	  csi[k][j-1][i]= csi[k][j-3][i];
	  eta[k][j-1][i]= eta[k][j-3][i];
	  zet[k][j-1][i]= zet[k][j-3][i];
  ...
  if (user->bctype[4]==7){
	...
	for (i=gxs; i<gxe; i++) {
	  k=0;
	  csi[k][j][i]= csi[k-2][j][i];
	  eta[k][j][i]= eta[k-2][j][i];
	  zet[k][j][i]= zet[k-2][j][i];
	  ...
	  csi[k-1][j][i]= csi[k-3][j][i];
	  eta[k-1][j][i]= eta[k-3][j][i];
	  zet[k-1][j][i]= zet[k-3][j][i];
	...
	if ((i == 0 &&  user->bctype[0]!=7) || i == mx-1 || (j==0 && user->bctype[2]!=7) || j==my-1 || (k==0 && user->bctype[4]!=7) || k==mz-1) {
	  ...
	  if (dir==0) {
		...
		val[3] -= 0.5*csi2*aj1*
	      ...
	      //j+1
	      if ((j==my-2 && user->bctype[2]!=7) || (j==my-3 && user->bctype[2]!=7) || (nvert[k][j][i]+nvert[k][j][i+1]>0.1)){
		val[4]=0.;
	      } else {
		val[4] -= g22*aj2*(nu_t+1/Re);
		...
		val[5] -= 0.5*csi2*aj1*
	      ...
	      //k+1
	      if ((k==mz-2 && user->bctype[4]!=7) || (k==mz-3 && user->bctype[4]!=7) || (nvert[k][j][i]+nvert[k][j][i+1]>0.1)){
		val[6]=0.;
	      } else {
		val[6] -= g33*aj2*(nu_t+1/Re);
		if (fabs(csi[k][j][i].z)>zero)
  ...
  while (((normdU>imp_atol && reldU>imp_rtol) || pseudot<1) && pseudot<imp_MAX_IT) {
	...
	for (dir=0;dir<3;dir++){
	  ...
	  for (k=zs; k<ze; k++) {
	    for (j=ys; j<ye; j++) {
	      for (i=xs; i<xe; i++) {
		if ((i == 0 &&  user->bctype[0]!=7) || i == mx-1 || (j==0 && user->bctype[2]!=7) || j==my-1 || (k==0 && user->bctype[4]!=7) || k==mz-1 || nvert[k][j][i]>0.1){
		  rb[k][j][i]=0.;
		  b[k][j][i].x=0.;
		  b[k][j][i].y=0.;
		  b[k][j][i].z=0.;
	  ...
	  if (dir==0 && user[bi].bctype[0]==7 && xs==0) {
	    for (k=lzs; k<lze; k++) {
	      for (j=lys; j<lye; j++) {
		i=0;
		ucont_i[k][j][i]=lucont_i[k][j][i-2];
	      }
	    }
	    ...
	    for (k=lzs; k<lze; k++) {
	      for (i=lxs; i<lxe; i++) {
		j=0;
		ucont_i[k][j][i]=lucont_i[k][j-2][i];
	      }
	    }
	    ...
	    for (j=lys; j<lye; j++) {
	      for (i=lxs; i<lxe; i++) {
		k=0;
		ucont_i[k][j][i]=lucont_i[k-2][j][i];
	      }
	    }
	  }
		     ...
		     FSInfo *fsi)
{
  ...
  PetscReal normdU=10.,normdU1[block_number],normdU0[block_number],reldU=1.,normdT, normF0[block_number], relF=1., normF1[block_number]; 
  ...
  while (((normdU>imp_atol && reldU>imp_rtol) || pseudot<1) && pseudot<imp_MAX_IT) {
	 ...
	 VecWAXPY(user[bi].Ucont, lambda[bi]*alfa[istage] * user[bi].dt * user[bi].st , user[bi].Rhs, user[bi].pUcont);
	 DMGlobalToLocalBegin(user[bi].fda, user[bi].Ucont, INSERT_VALUES, user[bi].lUcont);
	 DMGlobalToLocalEnd(user[bi].fda, user[bi].Ucont, INSERT_VALUES, user[bi].lUcont);
     ...
     for (bi=0; bi<block_number; bi++) {
       if (pseudot>1 && normF_bk[bi]>normF1[bi] && normdU_bk[bi]>normdU1[bi] && lambda[bi]>0.001) {
...
/*   PetscInt	mx = info.mx, my = info.my, mz = info.mz; */
  ...
  double norm;
  Vec    Norm;
  //  SNESGetFunctionNorm(user[bi].snes, &norm); Mohsen Nov 2015
...
PetscErrorCode  Implicit_FD_Jacobian(UserCtx *user, IBMNodes *ibm, FSInfo *fsi)  //Solver No.5
{
...
MatSetValues(PJ,1,&kk,1,&kk,&Unit,INSERT_VALUES);
  ...
  for (k=zs; k<ze; k++) {   //Immersed Boundary
    for (j=ys; j<ye; j++) {
      for (i=xs; i<lxe; i++) {
...
MatSetValues(PJ,1,&kk,1,&kk,&Unit,INSERT_VALUES);
  ...
  // MatCopy(*PJ,user[bi].PJ,SAME_NONZERO_PATTERN);
