...
extern PetscInt   cop, regime, fish,pizza, fishcyl, MHV, BHV, beam, LV, moveframe,rotateframe;
...
/* 	  fsi[ibi].F_z=F_z_tot; */
/* 	} */
/* 	PetscPrintf(PETSC_COMM_WORLD, "Total Forec and Momentum on surface body: M_x %le M_y %le M_z %le F_x %le F_y %le F_z %le \n",fsi[0].M_x,fsi[0].M_y,fsi[0].M_z,fsi[0].F_x,fsi[0].F_y,fsi[0].F_z); */
...
/* ==================================================================================             */
  if (rheology){
    ...
    for (ibi=0; ibi< NumberOfBodies; ibi++){
      ...
      //   PetscPrintf(PETSC_COMM_WORLD, "fsu[%d].S_new[0] %le \n",ibi,fsi[ibi].S_new[0]);
...
/*       } */
/*     } */
/*   } */else if (movefsi && moveframe==1){// && ti>tistart+3) {
    for (level = usermg->mglevels-1; level>=usermg->mglevels-1; level--) {
      user = usermg->mgctx[level].user;
      for (bi=0; bi<block_number; bi++) {
	/* 	if (immersed) { */
  ...
  if (rotatefsi && rotateframe==0) {
  ...
  } else if (rotatefsi && rotateframe==1){// && ti>tistart+3) {
    for (level = usermg->mglevels-1; level>=usermg->mglevels-1; level--) {
      user = usermg->mgctx[level].user;
      for (bi=0; bi<block_number; bi++) {
	/* 	if (immersed) { */
	for (ibi=0;ibi<NumberOfBodies;ibi++) {
	  ...
	  //Forced_Rotation(fsi, 1. ,2.*pi*0.1,user[bi].dt);
	  if (ti==tistart) {
	    fsi[ibi].S_ang_n[5]=St_exp;//this is for steady rotation
	    // u_c is frame translational speed
	    ...
	    if (ibi==0) {	    
	      CalcFrameVelocity(&user[bi],u_c,omega_c,a_c);
	      //CalcFrameVelocity_old(&user[bi],u_c,omega_c,a_c);
...
/* ==================================================================================             */
  ...
  if (cop) {
    ...
    for (bi=0; bi<block_number; bi++) {
      ...
      // regime==1 if self-propelled
/*       if (regime && ibi>0)  */
/* 	fsi[0].F_z +=fsi[ibi].F_z; */
      }//ibi
...
/*   For fish  */
/* ==================================================================================             */
  if (fish_c){// && rotateframe==1){// && ti>tistart+3) {
    for (level = usermg->mglevels-1; level>=usermg->mglevels-1; level--) {
      user = usermg->mgctx[level].user;
...
/* 	    omega_c.y=-fsi[ibi].S_ang_n[3]; */
/* 	    omega_c.z=-fsi[ibi].S_ang_n[5]; */
	    /* R is rotation matrix R=[cos sin; 
	                              -sin cos] 
	       position:   X= x_c + R(x_init - x_head - x_c)  
	       velocity: R dx_init = dX + R dx_head + dR(x_init - x_head - x_c)
	       dR (x_init - x_head - x_c)= dR R^T(X-x_c)
	       dR R^T(X-x_c)= omega x (X-x_c) = frame rotational velocity
	    */
    ...
    //    PetscPrintf(PETSC_COMM_WORLD, "MV_FRAME ROTATE!!!! %le %le %le %le %le\n",fsi[0].S_new[3],fsi[0].S_new[5],fsi[0].S_ang_n[2],fsi[0].S_ang_n[3],St_exp);
    if (rotateframe) 
...
//ibm_surface_out(&ibm[ibi],ti,ibi);
    ...
    for (ibi=0;ibi<NumberOfBodies;ibi++) {
      ...
      // find pojecting forces & power
/*       PetscBarrier(PETSC_NULL); */
/*       Projecting(&user[bi],&ibm[ibi]); */
/*       Projecting_VTKOut(&ibm[ibi],ibi,ti-1); */
/*       Projecting_Power(&ibm[ibi],&fsi[ibi],ti-1); */
/*       PetscBarrier(PETSC_NULL); */

      Calc_FSI_pos_intg(&fsi[ibi], &ibm[ibi], user[bi].dt) ;
	/* 	Flux_in=1.-fsi[ibi].S_new[5]; */
...
/* ==================================================================================             */
  if (wing) {
	...
	for (i=0; i<ibm[ibi].n_v; i++) {	  
	  ...
	  //	  ibm[ibi].u[i] = rotateT_xyz(ibm[ibi].u[i], alfa_c);
...
/*     *DoSCLoop = PETSC_TRUE; */
  ...
  if ((movefsi || rotatefsi || MHV || BHV || cop || fish || pizza || rheology) && STRONG_COUPLING && itr_sc<2) 
    *DoSCLoop = PETSC_TRUE;  
...
/* ==================================================================================             */
  ...
  if (implicit==1) {
    ...
    Implicit_MatrixFree(user, ibm, fsi);
     ...
     Implicit_FD_Jacobian(user, ibm, fsi);
  }else if (implicit==7) {
    ImpRK(user, ibm, fsi);
