...
extern PetscInt inletprofile, period;
extern PetscReal flux_flow[2500];
PetscReal U_bc;
...
#define M_PI 3.14159265358979323846264338327950288
 ...
 //  PetscPrintf(PETSC_COMM_WORLD,"FluxInSum: %le \n",FluxInSum);
...
PetscErrorCode FormBCS(UserCtx *user) {
  ...
  for (ttemp=0; ttemp<3; ttemp++) {
    ...
    if (user->bctype[5]==10) {
      ...
      VelDiffIn  = 1/3.*FluxDiff / (FarAreaInSum);
      if (fabs(FarAreaInSum) <1.e-6) VelDiffIn = 0.;
      
      VelDiffOut  = 2./3.* FluxDiff / (FarAreaOutSum) ;
    ...
    if (user->bctype[5]==4 || user->bctype[5]==14 || user->bctype[5]==20) {
      lArea=0.;
    //  PetscPrintf(PETSC_COMM_WORLD," Outlet at zmax boundary\n");
      if (ze == mz) {
	k=ze-1;
	FluxOut = 0;
	for (j=lys; j<lye; j++) {
	...
	for (j=lys; j<lye; j++) {
     ...
     // PetscPrintf(PETSC_COMM_WORLD," FluxOutSum : %le \n",FluxOutSum);
...
PetscErrorCode fluxin(UserCtx *user) {
  ...
  PetscPrintf(PETSC_COMM_WORLD, "Time step: %d Cyclic Non-dimentional time: %le flux-in %le intp%d between entires %d and %d (period: %d)\n",ti, t_rel, Flux_in, interpolated, i, i+1, ts_p_cycle);
...
PetscErrorCode SetInitialGuessToOne(UserCtx *user) {
  ...
  for (k=zs ; k<lze; k++) {
	...
	if (InitialGuessOne==2) {
	  ...
	  ucont[k][j][i].x = 0.;
	  ucont[k][j][i].y = 0.;	 
	  ucont[k][j][i].z = uin* sqrt( (zet[k][j][i].x) * (zet[k][j][i].x) +
					(zet[k][j][i].y) * (zet[k][j][i].y) +
					(zet[k][j][i].z) * (zet[k][j][i].z));
	}
      }
...
PetscErrorCode fluxin_novostia(UserCtx *user)
{
  ...
  if(!ti){
	...
	fscanf(fd, "%le\n",&flux_flow[i]);
      //
      MPI_Bcast(&flux_flow, 3000, MPI_DOUBLE, 0, PETSC_COMM_WORLD);
    }
    else {
      MPI_Bcast(&flux_flow, 3000, MPI_DOUBLE, 0, PETSC_COMM_WORLD);
    }
