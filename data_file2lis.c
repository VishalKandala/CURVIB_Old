...
PetscErrorCode FSI_list_output(FSInfo *FSinf, PetscInt ti)
{
  FILE *f;
  char filen[80];
...
/*   PetscFPrintf(PETSC_COMM_WORLD, f, "%d %le %le %le %le %le %le\n",ti,FSinf->S_new[4],FSinf->S_new[5],FSinf->F_z, FSinf->S_ang_n[0], FSinf->S_ang_n[1],FSinf->M_x); */
  PetscFPrintf(PETSC_COMM_WORLD, f, "%d %le %le %le\n",ti,FSinf->F_x,FSinf->F_y,FSinf->F_z);
  fclose(f);
  return(0);
}

PetscErrorCode ENG_DATA_Input(ENGinfo *eng, PetscInt ti)
{
  PetscInt tii;
  FILE *f;
  char filen[80];
  ...
  f = fopen(filen, "r");
  fscanf(f, "%d %le %le %le %le %le\n",&tii,&eng->Pw_x,&eng->Pw_y,&eng->Pw_z,&eng->F_z,&eng->A_tot);
...
PetscErrorCode FRC_DATA_Input(FRCinfo *frc, PetscInt ti)
{
  PetscInt tii;
  FILE *f;
  char filen[80];
...
PetscErrorCode FRC_list_output(FRCinfo *frc, PetscInt ti)
{
  FILE *f;
  char filen[80];
  sprintf(filen, "FRC_data");
  //sprintf(filen, "DATA_FRC%5.5d.dat",ti);
  f = fopen(filen, "a");
  PetscFPrintf(PETSC_COMM_WORLD, f, "%d %le %le %le %le %le %le %le %le %le\n",ti,frc->F_x,frc->F_y,frc->F_z,frc->Cp_x,frc->Cp_y,frc->Cp_z,frc->A_x,frc->A_y,frc->A_z);
  //  PetscFPrintf(PETSC_COMM_WORLD, f, "END of File\n");
...
PetscErrorCode ENG_list_output(ENGinfo *eng, PetscInt ti)
{
  FILE *f;
  char filen[80];
  sprintf(filen, "ENG_data");
...
int main(int argc, char **argv)
{
  ...
  for (ti=tistart;ti<=tiend;ti++) {
    if (FRC) {
      FRC_DATA_Input(&frc, ti);
      PetscPrintf(PETSC_COMM_WORLD, "ti, %d %e \n",ti, frc.F_z);
      FRC_list_output(&frc, ti);
    } else if (ENG) {
      ENG_DATA_Input(&eng, ti);
      PetscPrintf(PETSC_COMM_WORLD, "ti, %d %e \n",ti, eng.Pw_x) ;
      ENG_list_output(&eng, ti);
      //      ENG_DATA_output(&eng, tii);
   } else{
      FSI_DATA_Input(&FSI, ti);
      PetscPrintf(PETSC_COMM_WORLD, "ti, %d %e \n",tii, FSI.F_z);
      //    FSI_DATA_output(&FSI, tii, ibi);
      FSI_list_output(&FSI, ti);
