...
PetscErrorCode FormMetrics(UserCtx *user) {
  ...
  Cmpnts	***lcent,***cent, ***centx, ***centy, ***centz;
  ...
  if (user->cgrid && user->bctype[0]==7){
    ...
    //Mohsen Dec 2015
    DMDAVecGetArray(fda, user->lCent, &lcent);
    DMDAVecGetArray(fda, user->Cent, &cent);
    if (xs==0){
      for (k=zs; k<ze; k++) {
	for (j=ys; j<ye; j++) {
	  i=0;
	  cent[k][j][i]=lcent[k][j][i-2];
	}
      }
    }
    if (xe==mx){
    ...
    DMDAVecRestoreArray(fda, user->lCent, &lcent);
    DMDAVecRestoreArray(fda, user->Cent, &cent);
    DMGlobalToLocalBegin(fda, user->Cent, INSERT_VALUES, user->lCent);
    DMGlobalToLocalEnd(fda, user->Cent, INSERT_VALUES, user->lCent);
  ...
  /*   We need these centers for interpolation around immersed boundary  */
  if (!user->cgrid && user->bctype[0]==7){
 ...
 if (!user->cgrid && user->bctype[2]==7){
  ...
  VecStrideMax(user->Cent, 0, PETSC_NULL, &normX_max); // Get the max value along the 0 (x direction)
  VecStrideMax(user->Cent, 1, PETSC_NULL, &normY_max);// Get the max value along the 1 (y direction)
  VecStrideMax(user->Cent, 2, PETSC_NULL, &normZ_max); // Get the max value along the 2 (z direction)
  VecStrideMin(user->Cent, 0, PETSC_NULL, &normX_min);// Get the min value along the 0 (x direction)
  VecStrideMin(user->Cent, 1, PETSC_NULL, &normY_min);// Get the min value along the 1 (y direction)
  VecStrideMin(user->Cent, 2, PETSC_NULL, &normZ_min); // Get the min value along the 2 (z direction)
