...
extern PetscInt  cop, wing,NumberOfBodies, rheology, sediment, BHV, inv_flg;
...
PetscErrorCode ibm_read_Icem(IBMNodes *ibm, PetscInt ibi) {
  ...
  PetscReal     theta,yy,zz,Y,Z;
  PetscReal  char_length_x=1., char_length_y=1., char_length_z=1., z_dis=0.;

  PetscOptionsGetReal(PETSC_NULL, "-char_length_ibm", &cl, PETSC_NULL);     
  PetscOptionsGetReal(PETSC_NULL, "-char_length_x", &char_length_x, PETSC_NULL);
  PetscOptionsGetReal(PETSC_NULL, "-char_length_y", &char_length_y, PETSC_NULL);
  PetscOptionsGetReal(PETSC_NULL, "-char_length_z", &char_length_z, PETSC_NULL);
  PetscOptionsGetReal(PETSC_NULL, "-z_displacement", &z_dis, PETSC_NULL);

  char string[128];
  PetscOptionsGetInt(PETSC_NULL, "-rot", &rot, PETSC_NULL);
  PetscOptionsGetReal(PETSC_NULL, "-theta", &theta, PETSC_NULL);
  ...
  if(!rank) { // root processor read in the data
    ...
    //
    PetscReal xcg,ycg,zcg;
    if (ibi<3) {
      for (i=0; i<n_elmt; i++) {
	xcg=(x_bp[nv1[i]] + x_bp[nv2[i]] + x_bp[nv3[i]])/3.;
	ycg=(y_bp[nv1[i]] + y_bp[nv2[i]] + y_bp[nv3[i]])/3.;
	zcg=(z_bp[nv1[i]] + z_bp[nv2[i]] + z_bp[nv3[i]])/3.;
      }
    }
...
PetscErrorCode ibm_read_Ansys_vol(IBMVNodes *ibmv, PetscInt ibi)// added by Qiping
{
  ...
  if(!rank) { // root processor read in the data
	...
	ibmv->x_bp[i] = X/cl+CMz_c;
	ibmv->y_bp[i] = Y/cl+CMy_c;
	ibmv->z_bp[i] = Z/cl+CMz_c;

	ibmv->x_bp0[i] = X/cl+CMz_c;
	ibmv->y_bp0[i] = Y/cl+CMz_c;
	ibmv->z_bp0[i] = Z/cl+CMz_c;

	ibmv->x_bp_o[i] = X/cl+CMz_c;
	ibmv->y_bp_o[i] = Y/cl+CMz_c;
	ibmv->z_bp_o[i] = Z/cl+CMz_c;
	PetscPrintf(PETSC_COMM_WORLD, "xyz_bp %le %le %le\n", x_bp[i], y_bp[i], z_bp[i]);
      }
...
//---------------------------------------------------------------
PetscErrorCode calc_ibm_normal(IBMNodes *ibm, PetscInt ibi) {

  PetscInt   n1e, n2e, n3e, i;
  PetscReal  dx12, dy12, dz12, dx13, dy13, dz13, dr;
  PetscReal  ca=0., cx=0., cy=0., cz=0.; 
  ...
  for (i=0; i<ibm->n_elmt; i++) {
    ...
    cx += ibm->cent_x[i]*0.5*dr;
    cy += ibm->cent_y[i]*0.5*dr;
    cz += ibm->cent_z[i]*0.5*dr;
    ca += 0.5*dr;
...
//----------------------------------------------------------------------
PetscErrorCode  BroadcastIBM(IBMNodes *ibm) {

  PetscInt  rank, n_v=ibm->n_v;
  MPI_Comm_rank(PETSC_COMM_WORLD, &rank);
  if(!rank) {
    MPI_Bcast(ibm->x_bp, n_v, MPIU_REAL, 0, PETSC_COMM_WORLD);
    MPI_Bcast(ibm->y_bp, n_v, MPIU_REAL, 0, PETSC_COMM_WORLD);
...
//--------------------------------------------------------------------
PetscErrorCode BroadcastIBM_old(IBMNodes *ibm)
{
  PetscInt	rank, n_v=ibm->n_v;
  MPI_Comm_rank(PETSC_COMM_WORLD, &rank);
  if(!rank) {
