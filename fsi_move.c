...
{  
  ...
  ///////////////////////////////////////
  PetscReal uu=0.0, vv=0.0, ww=0.0;
  ...
  //
  M_x = -((M_x*uu)+(M_y*vv)+(M_z*ww));
  M_y = 0.0;
  M_z = 0.0;
  ///////////////////////////////////////
  PetscPrintf(PETSC_COMM_WORLD, "M_x %le M_y %le M_z %le red_vel %le damp %le mu_s %le \n",M_x,M_y,M_z,red_vel,damp,mu_s);
  
  // solve Ang mom equ using trapozoidal rule u(n+1)=u(n)+dt*F(n+1/2)
  if (dgf_ax) {
    S_ang_n[1]=S_ang_r[1]+ dt*(mu_s*M_x);
...
/*     S_ang_n[0]=S_ang_r[0]+0.5*(S_ang_n[1]+S_ang_r[1])*dt; //ang=ang_r+w_avedt */
  ...
  ////////////////////////////////
  /* if (fabs(FSinfo->S_ang_n[0])> fabs(max_angle)) { */
  /*   FSinfo->S_ang_n[0]= max_angle; */
  /*   FSinfo->S_ang_n[1]= 0.; */
  /* } */
  
  /* if (FSinfo->S_ang_n[0]> 0.0) { */
  /*   FSinfo->S_ang_n[0]= 0.0; */
  /*   FSinfo->S_ang_n[1]= 0.0; */
  /* } */
  /* //////////////////////////////// */
  
  /* // output values */
  /* PetscPrintf(PETSC_COMM_WORLD, "w_x %le theta_x %le w_y %le theta_y %le w_z %le theta_z %le \n",S_ang_n[1],S_ang_n[0],S_ang_n[3],S_ang_n[2],S_ang_n[5],max_angle); */
  return(0);
}
				 ...
				 PetscReal dt, PetscInt ibi)
{
  ...
  PetscReal pi = 3.14159265359, angle = -rot, R[4][4], x[4], xn[4], u, v, w, a, b, c, xd[3];
  PetscInt m,n;
  ...
  if (ibi == 1) {
    u = 1. ; v = 0.0;
    // xd[0] = 0.272384; xd[1] = -0.38098; xd[2]=0.253291;
  } else if (ibi == 2) {
    u = -0.5; v = 0.866025;
    //xd[0] = 0.145921; xd[1] = 0.509559; xd[2]=0.253291;
  } else if (ibi == 3) {
    u = -0.5; v = -0.86602;
    //xd[0] = -0.465447;  xd[1]= -0.043848; xd[2]=0.253291;
  }
  
  w = 0.; //Rotation vector
  a = x_c + xd[0]; b = y_c + xd[1]; c = z_c + xd[2]; //Rotation origin
  for (i=0; i<n_v; i++) { 
  ...
  for (i=0; i<n_v; i++) {
    ...
    ibm->u[i].x=(ibm->x_bp[i]- ibm->x_bp_o[i])/dt;//-FSinfo->S_real[0]);
    ibm->u[i].y=(ibm->y_bp[i]- ibm->y_bp_o[i])/dt;//-FSinfo->S_real[0]);
    ibm->u[i].z=(ibm->z_bp[i]- ibm->z_bp_o[i])/dt;//-FSinfo->S_real[0]);
    
    /* double pp1[3],U[3]; */
    /* pp1[0]= (ibm->x_bp[i]-x_c);pp1[1]= (ibm->y_bp[i]-y_c);pp1[2]= (ibm->z_bp[i]-z_c); */
    /* U[0]=u;U[1]=v;U[2]=w; */
    
    /* rx =pp1[0]-DOT(pp1,U)*U[0]; */
    /* ry =pp1[1]-DOT(pp1,U)*U[1]; */
    /* rz =pp1[2]-DOT(pp1,U)*U[2]; */
    /* // */
    /* wx=wx*U[0]; wy=wx*U[1]; wz=wx*U[2]; */
      
    /* ibm->u[i].x = rx;  //ry*wz-wy*rz  ; */
    /* ibm->u[i].y =ry;//-( rx*wz-wx*rz ); */
    /* ibm->u[i].z = rz;  //rx*wy-wx*ry  ; */
  }
...
PetscErrorCode ibmv_cent_of_mass_valve(IBMVNodes *ibmv, FSInfo *FSinfo,PetscInt ibi)
{
  ...
  /////////////////////
  PetscReal ratio=1300/1030.0;//density ratio
  /////////////////////
  ...
  for (i=0; i<n_elmt; i++) {
    ...
    J[0][0] +=((ibmv->cent_y[i]-FSinfo->y_c)*(ibmv->cent_y[i]-FSinfo->y_c)+(ibmv->cent_z[i]-FSinfo->z_c)*(ibmv->cent_z[i]-FSinfo->z_c))*ibmv->dV0[i];
    J[1][1] +=((ibmv->cent_x[i]-FSinfo->x_c)*(ibmv->cent_x[i]-FSinfo->x_c)+(ibmv->cent_z[i]-FSinfo->z_c)*(ibmv->cent_z[i]-FSinfo->z_c))*ibmv->dV0[i];
    J[2][2] +=((ibmv->cent_y[i]-FSinfo->y_c)*(ibmv->cent_y[i]-FSinfo->y_c)+(ibmv->cent_x[i]-FSinfo->x_c)*(ibmv->cent_x[i]-FSinfo->x_c))*ibmv->dV0[i];
    J[0][1] -=(ibmv->cent_x[i]-FSinfo->x_c)*(ibmv->cent_y[i]-FSinfo->y_c)*ibmv->dV0[i];
    J[0][2] -=(ibmv->cent_x[i]-FSinfo->x_c)*(ibmv->cent_z[i]-FSinfo->z_c)*ibmv->dV0[i];
    J[1][2] -=(ibmv->cent_z[i]-FSinfo->z_c)*(ibmv->cent_y[i]-FSinfo->y_c)*ibmv->dV0[i];
    //
    J[1][0]=J[0][1];
    J[2][0]=J[0][2];
    J[2][1]=J[1][2];
  ...
  FSinfo->mu_s=(J[0][0]*(u*u)+J[1][1]*(v*v)+J[2][2]*(w*w)+2.0*J[1][0]*(u*v)+2.0*J[1][2]*(v*w)+2.0*J[2][0]*(u*w))*ratio;
  //
  PetscPrintf(PETSC_COMM_WORLD, "mu:  %le \n",FSinfo->mu_s);
  //
  ibmv->V=V_tot;
  //
  ibmv->J[0][0]=J[0][0];
  ...
  PetscPrintf(PETSC_COMM_WORLD, "Centre of fsi: x_c  %le y_c %le z_c %le \n",ibmv->x_c,ibmv->y_c,ibmv->z_c);
