...
extern PetscInt immersed,MHV,LV;
extern PetscInt ti;
  ...
  if (!rank) {
	...
	for (k=0; k<mz; k++) {
	  if (nvert[k][j][i] > 0.1) {
	    if (!Blocked[j*mx+i]) {
	      KSKE[2*(j*mx+i)] = k;
  	    //  PetscPrintf(PETSC_COMM_WORLD,"k,j,i : %d, %d, %d \n",k,j,i);
...
PetscErrorCode MyNvertRestriction(UserCtx *user_h, UserCtx *user_c) // h for higher, c for coarser 
{
  //  DA		da = user_c->da, fda = user_c->fda;



  DMDALocalInfo	info = user_c->info;  // from the coarser grid.
  PetscInt	xs = info.xs, xe = info.xs + info.xm;
  PetscInt  	ys = info.ys, ye = info.ys + info.ym;
  PetscInt	zs = info.zs, ze = info.zs + info.zm;
  PetscInt	mx = info.mx, my = info.my, mz = info.mz;
...
PetscErrorCode PoissonSolver_MG(UserMG *usermg)
{
  ...
  if (immersed) {
	...
	MyNvertRestriction(&mgctx[l].user[bi], &mgctx[l-1].user[bi]);  // #1 Consecutive grid levels are being passed to the function.
  ...
  flg=immersed; // changed from 'immersed-1' to 'immersed'
  VolumeFlux_rev(&user[bi], &ibm_Flux, &ibm_Area, flg); // #2

  PoissonRHS(&(user[bi]), user[bi].B);  // #4
...
/*   PetscGlobalSum(&libm_area, ibm_Area, PETSC_COMM_WORLD); */
  ...
  PetscPrintf(PETSC_COMM_WORLD, "Correction  %le \n", correction);
