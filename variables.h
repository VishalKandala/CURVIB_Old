...
typedef struct {
  ...
  PetscReal  *pres_p, *tau0_p, *tauN_p;
  PetscReal  *Bvel_u_p, *Bvel_v_p, *Bvel_w_p;

  PetscReal  *pres, *tau0, *tauN;
  PetscReal  *Bvel_u, *Bvel_v, *Bvel_w;
  
  PetscReal  *pres_n, *tau0_n, *tauN_n;
  PetscReal  *Bvel_u_n, *Bvel_v_n, *Bvel_w_n;
...
typedef struct UserCtx {
  ...
  Vec	Phi;
  Vec	GridSpace;
  Vec	Nvert,Nvert1,Nvert2,Nvert3;
  Vec	Nvert_o;
  Vec   shrr;
  Vec   Itfc;//, ItfcP;
  BCS	Bcs;
  ...
  Vec	lGridSpace;
  Vec	lNvert, lNvert_o, lNFace,lnvert_test;
  Vec	lCent;
  Vec   lItfc;//, lItfcP;
...
/*   PetscReal	lAreaSum[6]; */
  ...
  // New
  Vec Ucat_sum,Ucat_sum0,Ucat_sum1,Ucat_sum2,Ucat_sum3;			// sum of u, v, w over time
  Vec Ucat_cross_sum;	// sum of uv, vw, wu
  Vec Ucat_square_sum;	// sum of uu, vv, ww
  Vec P_sum,P_sum0,P_sum1,P_sum2,P_sum3;				// sum of p
  Vec lSx, lSy, lSz, lS;  
  Vec lLM, lMM, lNM;
  //  std::vector<double> k_area;
  Vec	lNu_t, Nu_t, lF1;		// eddy viscosity, f1 constant for SST model
  Vec	lCs, CS;
...
PetscErrorCode fish_swim(IBMNodes *ibm, PetscReal time,PetscReal delti);
PetscErrorCode pizza_box_swim(IBMNodes *ibm, PetscReal time,PetscReal delti);
...
PetscErrorCode Projecting_Power(IBMNodes *ibm, FSInfo *fsi,PetscInt ti);
